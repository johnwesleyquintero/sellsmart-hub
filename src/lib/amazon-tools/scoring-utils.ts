...
const validateProductListing = (data: ProductListingData) => ({ success: true, data: data, error: null });
...
const applyThresholdScoring = (value: number, rule: ScoringRule): number => {
  ...
  const sortedThresholds = [...rule.thresholds].sort((a, b) => b.value - a.value);
...
const scoreDescription = (description: string | null | undefined): number => {
  ...
  const maxScore = descriptionRule.thresholds.reduce((max, t) => Math.max(max, t.score), 0);
    ...
    if (score < 7) {
      switch (key) {
        case 'title':
          ...
          messages[key] = config.title.thresholds.find(t => t.score < 7)?.message || 'Title needs improvement.';
          break;
        case 'bulletPoints':
          suggestions.push(
        ...
        case 'description':
          ...
          messages[key] = config.description.thresholds.find(t => t.score < 7)?.message || 'Description needs improvement.';
          break;
        case 'images':
          suggestions.push(
            'Include 7-9 high-quality images showing product features and benefits.',
          );
          messages[key] = config.images.thresholds.find(t => t.score < 7)?.message || 'Add more high-quality images.';
          break;
        case 'reviews':
          suggestions.push(
        ...
        case 'aPlus':
          ...
          messages[key] = config.aPlus.thresholds.find(t => t.value === 0)?.message || 'Consider adding A+ Content.';
          break;
        case 'fulfillment':
          suggestions.push(
            'Consider FBA to improve visibility and customer trust.',
          );
          messages[key] = config.fulfillment.thresholds.find(t => t.value === 0)?.message || 'Consider using FBA.';
          break;
      }
    } else {
        // Optionally add positive messages for scores >= 7
        switch (key) {
            case 'title': messages[key] = config.title.thresholds.find(t => t.score >= 7)?.message || 'Title is good.'; break;
            case 'description': messages[key] = config.description.thresholds.find(t => t.score >= 7)?.message || 'Description is good.'; break;
            case 'images': messages[key] = config.images.thresholds.find(t => t.score >= 7)?.message || 'Image count is good.'; break;
            case 'aPlus': messages[key] = config.aPlus.thresholds.find(t => t.value === 1)?.message || 'A+ Content present.'; break;
            case 'fulfillment': messages[key] = config.fulfillment.thresholds.find(t => t.value === 1)?.message || 'FBA fulfillment is used.'; break;
            // Add messages for bulletPoints and reviews if desired
            case 'bulletPoints': messages[key] = 'Bullet points are well-structured.'; break;
            case 'reviews': messages[key] = 'Reviews profile is strong.'; break;
        }
    }
  });
...
export const calculateProductScore = (
  ...
  try {
    ...
    const breakdown = {
      ...
      bulletPoints: config.bulletPoints.customScoring?.(data.bulletPoints || []) || 0, // Pass array or empty array
    ...
    throw new CalculationError(`Failed to calculate product score: ${(error as Error).message}`);
  }
};

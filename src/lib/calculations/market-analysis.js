export class MarketAnalysis {
    static calculateMarketMetrics(data) {
        ...
        if (historicalVolume && historicalVolume.length >= 2) {
            const recentChange = ((historicalVolume[historicalVolume.length - 1] -
    ...
    static analyzeCompetitor(metrics, marketAverages) {
        ...
        if (marketShareScore > 1.5)
            competitivePosition = 'leader';
        else if (marketShareScore > 0.7)
            competitivePosition = 'challenger';
        else
            competitivePosition = 'follower';
        return Object.assign(Object.assign({}, metrics), { salesVelocity, marketShare: marketShareScore, competitivePosition });
    }

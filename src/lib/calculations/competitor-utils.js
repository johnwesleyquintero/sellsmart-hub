export class CompetitorUtils {
    static analyzePricing({ competitorPrices, targetMargin, marketTrend, }) {
        // Remove outliers
        ...
        const confidence = Math.min((validPrices.length / competitorPrices.length) *
            100 *
            (1 - priceSpread / maxPrice), 100);
        return {
            strategy,
            suggestedPrice,
            priceRange: {
    ...
    static analyzeMarketPosition(metrics) {
        ...
        if (avgRating > 4.5)
            competitiveAdvantage.push('High product quality');
        if (avgReviews > 1000)
            competitiveAdvantage.push('Strong market presence');
        if (metrics.length < 10)
            competitiveAdvantage.push('Limited competition');
        // Identify threats
        const threats = [];
        if (avgPrice < 20)
            threats.push('Price war risk');
        if (metrics.length > 50)
            threats.push('Saturated market');
        if (avgRating < 4.0)
            threats.push('Quality concerns');
        // Identify opportunities
        const opportunities = [];
        if (avgReviews < 500)
            opportunities.push('Market growth potential');
        if (avgRating < 4.5)
            opportunities.push('Quality differentiation');
        if (segment === 'premium')
            opportunities.push('Brand development');
        return {
            segment,

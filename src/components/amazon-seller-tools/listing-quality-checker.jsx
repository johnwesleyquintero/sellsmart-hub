...
export default function ListingQualityChecker() {
        ...
        reader.onload = (e) => {
            ...
            try {
                ...
                Papa.parse(content, {
                    header: true,
                    skipEmptyLines: true,
                    complete: (results) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        const processedData = yield Promise.all(results.data.map((row) => __awaiter(this, void 0, void 0, function* () {
                            var _a, _b;
                            const keywords = ((_a = row.keywords) === null || _a === void 0 ? void 0 : _a.split(',').map((k) => k.trim()).filter(Boolean)) || [];
                            const keywordAnalysis = yield KeywordIntelligence.analyzeBatch(keywords);
                            return {
                                product: row.product,
                                title: row.title,
                                description: row.description,
                                bulletPoints: (_b = row.bullet_points) === null || _b === void 0 ? void 0 : _b.split(';').filter(Boolean),
    ...
    return (<div className="space-y-6">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="flex gap-2">
                    <Input value={asin} onChange={(e) => setAsin(e.target.value)} placeholder="Enter ASIN (e.g., B08N5KWB9H)"/>
      ...
      {listings.length > 0 && (<div className="space-y-4">
          {listings.map((listing, index) => (<Card key={index}>
              <CardContent className="p-4">
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-lg font-medium">{listing.product}</h3>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium">Quality Score:</span>
                    <Badge variant={(listing.score || 0) >= 80
                    ? 'default'
                    : (listing.score || 0) >= 50
                        ? 'secondary'
                        : 'destructive'}>
                ...
                <div className="mb-4 grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <div className="space-y-2 rounded-lg border p-3">
                      <div className="flex justify-between">
                        ...
                        <span className="flex items-center text-sm">
                          {listing.title ? (<CheckCircle className="mr-1 h-4 w-4 text-green-500"/>) : (<XCircle className="mr-1 h-4 w-4 text-red-500"/>)}
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="flex items-center text-sm">
                          {listing.description ? (<CheckCircle className="mr-1 h-4 w-4 text-green-500"/>) : (<XCircle className="mr-1 h-4 w-4 text-red-500"/>)}
                    ...
                    listing.bulletPoints.length >= 5 ? (<CheckCircle className="mr-1 h-4 w-4 text-green-500"/>) : (<XCircle className="mr-1 h-4 w-4 text-red-500"/>)}
                          {listing.bulletPoints
                    ...
                    : 'Missing'}
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="flex items-center text-sm">
                          {listing.images && listing.images >= 7 ? (<CheckCircle className="mr-1 h-4 w-4 text-green-500"/>) : (<XCircle className="mr-1 h-4 w-4 text-red-500"/>)}
                          {listing.images
                    ...
                    : 'Missing'}
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="flex items-center text-sm">
                          {listing.keywords && listing.keywords.length >= 5 ? (<CheckCircle className="mr-1 h-4 w-4 text-green-500"/>) : (<XCircle className="mr-1 h-4 w-4 text-red-500"/>)}
                          {listing.keywords
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {listing.issues && listing.issues.length > 0 && (<div className="space-y-1">
                      ...
                      {listing.suggestions &&
                    listing.suggestions.length > 0 && (<div className="space-y-1">
                            <h5 className="text-xs font-medium text-blue-600 dark:text-blue-400">
                              Suggestions:
                            </h5>
                            <ul className="list-inside list-disc space-y-1 text-sm">
                              {listing.suggestions.map((suggestion, i) => (<li key={i}>{suggestion}</li>))}
                            </ul>
                          </div>)}
                    </div>
                  </div>

...
const KeywordTrendAnalyzer = () => {
    ...
    const handleFileUpload = useCallback((data) => __awaiter(void 0, void 0, void 0, function* () {
        try {
      ...
      {chartData.length > 0 && (<ResponsiveContainer width="100%" height={400}>
          <LineChart data={chartData}>
            ...
            {Object.keys(chartData[0])
                .filter((key) => key !== 'name')
                .map((key) => (<Line key={key} type="monotone" dataKey={key} stroke={`#${Math.floor(Math.random() * 16777215).toString(16)}`}/>))}
          </LineChart>
        </ResponsiveContainer>)}
    </div>);
};

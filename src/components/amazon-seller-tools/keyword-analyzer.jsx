...
import { AlertCircle, Download, FileText, Info, Search, Upload, } from 'lucide-react';
...
export default function KeywordAnalyzer() {
    ...
    const handleFileUpload = (event) => __awaiter(this, void 0, void 0, function* () {
        ...
        Papa.parse(file, {
            header: true,
            skipEmptyLines: true,
            complete: (result) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const processedData = yield Promise.all(result.data
                        .filter((item) => item.product && item.keywords)
                        .map((item) => __awaiter(this, void 0, void 0, function* () {
                        const keywordArray = typeof item.keywords === 'string'
                            ? item.keywords
                                .split(',')
                                .map((k) => k.trim())
                        ...
                        const analysis = yield KeywordIntelligence.analyzeBatch(keywordArray || []);
                        return {
    ...
    const handleExport = () => {
        ...
        const exportData = products.map((product) => {
            ...
            return ({
                ...
                suggestions: (_a = product.suggestions) === null || _a === void 0 ? void 0 : _a.join(', '),
            ...
            <div className="flex flex-col items-center justify-center gap-4 p-6 text-center">
              ...
              <div className="w-full">
                <label className="relative flex w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-primary/40 bg-background p-6 text-center hover:bg-primary/5">
                  ...
                  <input type="file" accept=".csv" className="hidden" onChange={handleFileUpload} disabled={isLoading} ref={fileInputRef}/>
                </label>
                <div className="flex justify-center mt-4">
                  <SampleCsvButton dataType="keyword" fileName="sample-keyword-analyzer.csv"/>
                </div>
                {products.length > 0 && (<Button variant="outline" className="w-full mt-4" onClick={clearData}>
                    Clear Data
                  </Button>)}
              </div>
            </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="flex gap-2">
                    <Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Enter product or keyword"/>
            ...
            {products.map((product, index) => (<Card key={index}>
                <CardContent className="p-4">
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    {product.searchVolume && (<div className="flex items-center gap-2">
                        ...
                        <Badge variant={product.competition === 'High'
                        ? 'destructive'
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {product.keywords.map((keyword, i) => (<Badge key={i} variant="outline">
                            {keyword}
                          </Badge>))}
                      </div>
                      {product.searchVolume && (<div className="h-80 w-full">
                          <h4 className="mb-2 text-sm font-medium">
                            Keyword Performance
                          </h4>
                    ...
                    {product.suggestions && (<div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {product.suggestions.map((keyword, i) => (<Badge key={i} variant="secondary">
                              {keyword}
                            </Badge>))}
                        </div>
                      </div>)}

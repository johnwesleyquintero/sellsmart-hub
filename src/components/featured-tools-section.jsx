...
export default function FeaturedToolsSection() {
    ...
    return (<section id="tools" className="py-16 md:py-24 bg-gradient-to-b from-white to-blue-50 dark:from-gray-900 dark:to-gray-800">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden group">
          <div className="p-6 border-b border-gray-200 dark:border-gray-700">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <div className="flex flex-col gap-8">
                <div className="flex justify-center">
                  <TabsList className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {tools.slice(0, 4).map((tool) => (<TabsTrigger key={tool.id} value={tool.id} className="flex items-center gap-2 hover:bg-accent/50 transition-colors duration-300">
                        {tool.icon}
                        <span className="hidden md:inline">{tool.name}</span>
                        {tool.status === 'beta' && (<Badge variant="secondary" className="ml-1 px-1.5 py-0.5 text-xs">
                            Beta
                          </Badge>)}
                      </TabsTrigger>))}
                  </TabsList>
                ...
                <div className="flex justify-center">
                  <TabsList className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {tools.slice(4, 8).map((tool) => (<TabsTrigger key={tool.id} value={tool.id} className="flex items-center gap-2 hover:bg-accent/50 transition-colors duration-300">
                        {tool.icon}
                        <span className="hidden md:inline">{tool.name}</span>
                      </TabsTrigger>))}
                  </TabsList>
                ...
                <div className="flex justify-center">
                  <TabsList className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {tools.slice(8).map((tool) => (<TabsTrigger key={tool.id} value={tool.id} className="flex items-center gap-2 hover:bg-accent/50 transition-colors duration-300">
                        {tool.icon}
                        <span className="hidden md:inline">{tool.name}</span>
                      </TabsTrigger>))}
                  </TabsList>
              ...
              {tools.map((tool) => (<TabsContent key={tool.id} value={tool.id} className="mt-4">

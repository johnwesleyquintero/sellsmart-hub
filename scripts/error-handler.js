...
function analyzeErrors(errorOutput) {
  ...
  errors.forEach(error => {
    ...
    if (error.includes('Cannot find module') || error.includes('Cannot resolve module')) {
      categorized.add(ERROR_CATEGORIES.PATH);
      matched = true;
    }

    // Type definition errors
    if (error.includes('could not find a declaration file') || error.includes('requires types')) {
      categorized.add(ERROR_CATEGORIES.TYPES);
      matched = true;
    }

    // Import and export errors
    if (error.includes('has no exported member') || error.includes('Module not found')) {
    ...
    // Dependency errors
    if (error.includes('requires types') || error.includes('peer dependencies')) {
  ...
  return Array.from(categorized).sort((a, b) => 
    (errorCounts.get(b) || 0) - (errorCounts.get(a) || 0)
  );

...
export class AmazonSellerApi {
  ...
  private async refreshAccessToken(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new AmazonSellerApiError('Failed to refresh access token', response.status)
      }
  ...
  private async makeRequest<T>(endpoint: string, method: string = 'GET', body?: any): Promise<T> {
    ...
    try {
      const response = await fetch(`https://sellingpartnerapi-${this.config.region}.amazon.com${endpoint}`, {
        method,
        headers: {
  ...
  async getInventory(asin: string): Promise<InventoryResponse> {
    return this.makeRequest<InventoryResponse>(`/inventory/v1/products/${asin}`)
  }
  ...
  async batchGetFBAFees(asins: string[]): Promise<FBAFeeResponse[]> {
    return this.makeRequest<FBAFeeResponse[]>('/fba/fees/v1/products', 'POST', { asins })
  }

  async batchGetInventory(asins: string[]): Promise<InventoryResponse[]> {
    return this.makeRequest<InventoryResponse[]>('/inventory/v1/products', 'POST', { asins })
  }

  async batchGetPricing(asins: string[]): Promise<PricingResponse[]> {
    return this.makeRequest<PricingResponse[]>('/pricing/v1/products', 'POST', { asins })
  }
}
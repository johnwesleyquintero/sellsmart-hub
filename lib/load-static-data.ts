...
export async function loadStaticData<T extends Record<string, any>>(
  ...
  if (file === 'projects') {
    return (await import('../data/portfolio-data/projects.json')).default as T;
  }
  if (file === 'blog') {
    return (await import('../data/portfolio-data/blog.json')).default as T;
  }
  if (file === 'case-studies') {
    return (await import('../data/portfolio-data/case-studies.json')).default as T;
  }
  if (file === 'changelog') {
    // Cast to unknown first to satisfy TypeScript conversion
    return (await import('../data/portfolio-data/changelog.json')).default as unknown as T;
  }
  if (file === 'experience') {
    return (await import('../data/portfolio-data/experience.json')).default as T;
  }

...
export function calculateAmazonMetrics(data: {
  ...
  const profitMargin = ((data.sales - data.cost - data.adSpend) / data.sales) * 100;
...
function calculateSeasonalityScore(category?: string): number {
  ...
  const seasonalFactors: Record<string, number[]> = {
    ...
    'Home & Kitchen': [1.1, 0.9, 1.0, 1.2, 1.3, 1.2, 1.0, 1.1, 1.2, 1.3, 1.5, 1.6],
    // Add more categories...
  };
...
function calculateCompetitiveIndex(metrics: {
  ...
  const benchmarks: Record<string, { acos: number; ctr: number; conversionRate: number }> = {

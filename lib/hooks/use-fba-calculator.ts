...
export function useFBACalculator({ onError }: UseFBACalculatorProps = {}) {
  ...
  const calculateWithRealFees = async (products: ProductData[]) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to fetch FBA fees'
      onError?.(errorMessage)
      // Fallback to local calculation if API fails
      return products.map(product => ({
        ...product,
        profit: product.price - product.cost - product.fees,
        roi: ((product.price - product.cost - product.fees) / product.cost) * 100,
        margin: ((product.price - product.cost - product.fees) / product.price) * 100
      }))
    } finally {
      setIsLoading(false)
    }

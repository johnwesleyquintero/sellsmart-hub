...
- **Installation:** Ensure npm (version >= 1.0.0 recommended) is installed globally or available in your environment.
- **Dependency Management:** Use `npm install`, `npm add <package>`, `npm remove <package>`.
- **Lockfile:** Commit the `npm.lockb` file to version control to ensure deterministic installs.
...
### 2.3. npm Configuration Quality Checks (`npm run cq`)

Run `npm run cq` periodically or in CI pipelines to:

- Verify npm version compatibility.
- Ensure the integrity and usage of `npm.lockb`.

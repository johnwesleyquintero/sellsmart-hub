...
This project utilizes **npm** as the primary package manager.

- **Installation:** Ensure npm (latest stable version recommended) is installed globally (`npm install -g npm`) or available in your environment.
- **Dependency Management:** Use `npm install`, `npm add <package>`, `npm remove <package>`.
- **Lockfile:** Commit the `package-lock.json` file to version control to ensure deterministic installs across all environments. **This is the single source of truth for dependencies.**
- **Consistency:** **All project documentation (`README.md`, `CONTRIBUTING.md`, etc.) must reflect the use of `npm`.** (Action Item: Update other docs).
...
- Use `npm run env:pull` (if configured) to sync environment variables from a secure source like Vercel.

### 2.3. npm Configuration Quality Checks (`cq` script)

Run quality checks (`npm run cq`) periodically or in CI pipelines to:

- Verify npm version compatibility.
- Ensure the integrity of `package-lock.json`.
- Validate that `package.json` scripts are compatible with npm.
- Check npm's module resolution behavior.
...
- `package-lock.json`: **The** dependency lockfile.
...
- **Commits:**
  ...
  - Ensure commits pass all pre-commit hooks or CI checks (`npm run check:all`).
- **Pull Requests (PRs):**
...
1.  **Resolve Package Manager Inconsistency:** Ensure `npm` is consistently documented and used everywhere (`.rules.md`, `README.md`, `CONTRIBUTING.md`). (This document updated, others pending).
2.  **Align `README.md` Tool Status:** Update the `README.md` "Tools Documentation" table to accurately reflect the _actual_ status based on `TODO.md`.
...
5.  **Develop Core Content:** Create initial blog posts for _functional_ Seller Tools. (`TODO` related to item 11 in `NEW IDEA.MD`)
...
- **Linting & Formatting:** Strictly adhere to ESLint/Prettier. Run `npm run check:all` before committing.
...
Scripts are run via `npm run <script_name>`.
...
|                       | `start`                 | Starts production server.                                                       | Requires `npm build` first.               |
...
|                       | `clean:all`             | Removes `.next` and `node_modules`.                                             | Requires `npm install` afterwards.        |
|                       | `analyze`               | Analyzes production bundle size.                                                | Requires `npm build` first.               |
...
| **Utilities**         | `cq`                    | Runs npm configuration quality checks (version, lockfile integrity).            |                                           |
|                       | `env:pull`              | Syncs environment variables from remote source (e.g., Vercel).                  | Requires configuration.                   |
|                       | `postinstall`           | Script run automatically after `npm install` (e.g., for patching dependencies). | Defined in `package.json`.                |
|                       | `cq`                    | Executes predefined development tasks and automation scripts.                   | Windows batch script for task automation. |
